...
load('step0_output-qc/phenotyped_data_P04_primary_reg05.rdata')
...
for(sam in unique(phenotyped_data_P04_primary_reg05$sample)){
  phenotyped_data_P04_primary_reg05 %>%
  ...
  for(reg in unique(d0$region)){
    ...
    cl <- makePSOCKcluster(4) ## define number of cores
    ...
    reg0 = foreach (j = 1:(length(comlevels)/2), .combine= rbind) %dopar%{
      ...
      mean(p2[!p2$trans %in% c(Inf, NaN, NA) & p2$radius> 5 & p2$radius < 30,]$trans) -> Gmean30
      ...
      df_out = data.frame(sample = sam, region = reg, path_region = 'CT',
                          ...
                          Gmean30=Gmean30, Gmean100 = Gmean100)
      df_out
