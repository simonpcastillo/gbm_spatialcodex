...
## Last modified (ddmmyyyy): 15092024


pacman::p_load(foreach,doParallel,parallel, dplyr, reshape2,stringr,ClusterR, data.table, BiocManager)
...
ss_all_gat = phenotyped_data_P04_primary_reg05 
...
for(sam in unique(ss_all_gat$sample)){
  ...
  for(reg in unique(d0$region)){
    ...
    d2= d1 %>% dplyr::select(sample, x, y, size)
